...
**原文配图是卡片的主视觉**（issue #52）：正文抽取同时挑出原文里有信息量的图（实验曲线、架构图、流程图、
推文里的跑分截图），过滤掉 logo、头像、图标、推荐位缩略图；`noscript` 里交互图表的静态版也收。定稿时每条最多
下载 4 张到 `$QLY_DATA_DIR/media/`（按文件头认格式，单张超过 2.5MB 不要），页面用相对路径引用，发布时随页上传
——artifact 不放行外站图片。我们自己的提炼（结论 / 数字 / 逻辑链）退到辅助位；原文有图时，日报页不再画自绘对比图。

**侧边栏**（issue #51）：左栏「本期导读」按方向列出每条标题，点击跳转、滚动时高亮当前条目、已读打勾 + 进度，
关注开关也在这里；右栏「30 秒读完今天」是每条的一句话结论。窄屏时左栏收成顶部方向条，速览折叠在正文顶部。
